...
if [[ -n ${REPO-} ]]; then
	ORG="${ORG:-${REPO%%/*}}"
	GH_API_PATH="/repos/${REPO}/code-scanning/alerts?state=${STATE}"
	SCOPE="repo"
else
	ORG="${ORG:?set ORG to the target organization login}"
	GH_API_PATH="/orgs/${ORG}/code-scanning/alerts?state=${STATE}"
	SCOPE="org"
fi

if [[ -n ${TOOL_NAME-} ]]; then
	GH_API_PATH="${GH_API_PATH}&tool_name=${TOOL_NAME}"
fi
  ...
  def map_alert($org):
    {
      ...
      title: (.rule.description // .rule.name // .rule.id),
      external_id: .rule.id,
      created_at: .created_at,
      url: .html_url
    };
  def by_severity($alerts):
    ($alerts | map(select(.severity != null)) | group_by(.severity) | map({(.[0].severity): length}) | add // {});
